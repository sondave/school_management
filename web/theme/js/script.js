...
function csvEscape(value) {
	var text = cleanExportText(value).replace(/"/g, '""');
	return '"' + text + '"';
}
...
function getTableExportData($table) {
	...
	$table.find('thead th:visible').each(function(index) {
		if (!$(this).hasClass('no-export')) {
			exportColumnIndexes.push(index);
			headers.push(cleanExportText($(this).text()));
		}
	});

	var rows = [];
	dt.rows({ search: 'applied', order: 'applied', page: 'all' }).every(function() {
		var $tr = $(this.node());
		var row = [];

		exportColumnIndexes.forEach(function(idx) {
			var $cell = $tr.children().eq(idx);
			row.push(cleanExportText($cell.text()));
		});
...
function exportTableCsv($table, fileName) {
	var data = getTableExportData($table);
...
function exportTableExcel($table, fileName) {
	var data = getTableExportData($table);
	...
	downloadBlobFile(blob, fileName + '.xls');
}

function initDataNewTables($scope) {
	var $root = $scope && $scope.length ? $scope : $(document);
	var $tables = $root.find('.datanew');
	...
	$tables.each(function() {
		...
		var hasOnlyColspanRow = $rows.length === 1 && $rows.find('td[colspan]').length === 1;
		var enableExport = $table.data('export') === 1 || $table.data('export') === '1';
		var domLayout = 'ftrlpi';

		if (hasOnlyColspanRow || $.fn.DataTable.isDataTable($table)) {
			return;
		}
		...
		$table.DataTable({
			...
			initComplete: function(settings, json) {
				...
				var exportMenuId = 'export-menu-' + tableId;
				...
				if (enableExport && filter.length) {
					wrapper.find('.dt-custom-export').remove();
					var exportDropdown = '<div class="dropdown dt-custom-export ms-2">'
						+ '<button class="btn btn-outline-primary btn-sm dropdown-toggle" type="button" id="' + exportMenuId + '" data-bs-toggle="dropdown" aria-expanded="false">Export</button>'
						+ '<ul class="dropdown-menu dropdown-menu-end" aria-labelledby="' + exportMenuId + '">'
						+ '<li><a class="dropdown-item dt-export-csv" href="#">CSV</a></li>'
						+ '<li><a class="dropdown-item dt-export-excel" href="#">Excel</a></li>'
						+ '</ul>'
						+ '</div>';

					filter.addClass('d-flex align-items-center gap-2 justify-content-end');
					filter.append(exportDropdown);
				}
			},
		});
	});
...
$(document).ready(function(){
	...
	// Datatable
	initDataNewTables($(document));

	$(document).on('click', '.dt-export-csv', function(e) {
		e.preventDefault();
		var $table = $(this).closest('.dataTables_wrapper').find('table.datanew').first();
		if ($table.length) {
			exportTableCsv($table, 'sms-notifications');
		}
	});

	$(document).on('click', '.dt-export-excel', function(e) {
		e.preventDefault();
		var $table = $(this).closest('.dataTables_wrapper').find('table.datanew').first();
		if ($table.length) {
			exportTableExcel($table, 'sms-notifications');
		}
	});

	$(document).on('pjax:end', function(event) {
		initDataNewTables($(event.target));
