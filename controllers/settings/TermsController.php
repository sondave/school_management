...
class TermsController extends \app\controllers\Controller
{
    ...
    public function actionIndex(): string
    {
        $dataProvider = new ActiveDataProvider([
            'query' => Term::find()->with(['academicYear'])->orderBy(['id' => SORT_DESC]),
            'pagination' => [
                'pageSize' => 20,
            ],
        ]);
    ...
    public function actionCreate(): Response|array|string
    {
        ...
        if ($model->load($request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Term saved successfully.');
    ...
    public function actionUpdate(int $id): Response|array|string
    {
        ...
        if ($model->load($request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Term updated successfully.');
            return $this->redirect(['index']);
        }
    ...
    public function actionDelete(int $id): Response|array
    {
        ...
        Yii::$app->session->setFlash('success', 'Term deleted successfully.');
        return $this->redirect(['index']);
    }

    protected function findModel(int $id): Term
    {
