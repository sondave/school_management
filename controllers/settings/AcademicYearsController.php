...
class AcademicYearsController extends \app\controllers\Controller
{
    ...
    public function actionIndex(): string
    {
        $dataProvider = new ActiveDataProvider([
            'query' => AcademicYear::find()->orderBy(['id' => SORT_DESC]),
            'pagination' => [
                'pageSize' => 20,
            ],
        ]);
    ...
    public function actionCreate(): Response|array|string
    {
        ...
        if ($request->isAjax && $model->load($request->post())) {
            Yii::$app->response->format = Response::FORMAT_JSON;
            if ($model->save()) {
                return ['success' => true, 'message' => 'Academic year saved successfully.'];
            }
        ...
        if ($model->load($request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Academic year saved successfully.');
    ...
    public function actionUpdate(int $id): Response|array|string
    {
        ...
        if ($model->load($request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Academic year updated successfully.');
            return $this->redirect(['index']);
        }
    ...
    public function actionDelete(int $id): Response|array
    {
        ...
        Yii::$app->session->setFlash('success', 'Academic year deleted successfully.');
        return $this->redirect(['index']);
    }

    protected function findModel(int $id): AcademicYear
    {
