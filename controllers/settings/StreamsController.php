...
class StreamsController extends \app\controllers\Controller
{
    ...
    public function actionIndex(): string
    {
        $dataProvider = new ActiveDataProvider([
            'query' => Stream::find()->orderBy(['id' => SORT_DESC]),
            'pagination' => [
                'pageSize' => 20,
            ],
        ]);
    ...
    public function actionCreate(): Response|array|string
    {
        ...
        if ($model->isNewRecord && $model->status === null) {
            $model->status = Stream::STATUS_ACTIVE;
        }
        ...
        if ($model->load($request->post()) && $model->save()) {
            Yii::$app->session->setFlash('success', 'Stream saved successfully.');
    ...
    protected function findModel(int $id): Stream
    {
